...
- **@goodie-ts/hono**: Multi-runtime `EmbeddedServer` (Node, Bun, Deno). `ServerConfig` gains `runtime` field (`ServerRuntime` type: `'node' | 'bun' | 'deno'`). When `server.runtime` is `'cloudflare'`, `startServer()` and `EmbeddedServer` import are omitted from codegen — use `createRouter(ctx)` directly. **Breaking:** `EmbeddedServer.listen()` is now `async` — callers must `await` it.
