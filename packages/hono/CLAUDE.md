...
- **`visitClass`** — detects `@Controller(basePath)` and initializes controller metadata on the bean
...
The transformer core knows nothing about HTTP methods, routes, or validation. It only knows `@Controller` implies singleton bean (same as `@PostProcessor` implies singleton). All route scanning and codegen is fully owned by this package — following the Micronaut pattern where HTTP processing lives in the framework module, not the DI core.
