...
1. **Compile time:** The `createEventsPlugin()` transformer plugin detects classes extending `ApplicationEventListener` via `visitClass`. It marks them with `__isEventListener` metadata. In `beforeCodegen`, it adds `baseTokenRefs` pointing to `ApplicationEventListener` (so `getAll()` can discover them) and synthesizes an `EventBus` component that depends on `ApplicationContext`.
2. **Runtime:** `EventBus` takes `ApplicationContext` as a constructor dep. In `init()` (via `@OnInit`), it calls `ctx.getAllAsync(ApplicationEventListener)` to discover all listener components, groups them by `eventType`, and sorts by `order`. When `publish(event)` is called, it matches by `event.constructor` identity, filters by `supports()`, then dispatches sequentially.
...
- **Always-present EventBus** -- The plugin creates an `EventBus` component even when zero listeners are found, allowing `EventPublisher` injection in publish-only code.
- **Eager singleton** -- `EventBus` is created eagerly so listener discovery happens during context startup.
- **`baseTokenRefs`** -- `EventBus` registers under `EventPublisher` for interface injection. Listener components register under `ApplicationEventListener` for `getAll()` discovery.
- **ApplicationContext self-registration** -- `ApplicationContext` registers itself as a component so both `EventBus` and `SchedulerService` can inject it as a constructor dep.
