...
- **`ComponentDefinition<T>`** — Bean descriptor with token, scope, factory, dependencies, and metadata
...
- `ApplicationContext.create(defs, options?)` — async factory, evaluates conditional rules (`@ConditionalOnProperty`, `@ConditionalOnEnv`, `@ConditionalOnMissing`) at runtime, then topo-sorts and validates the dependency graph
