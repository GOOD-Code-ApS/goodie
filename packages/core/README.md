...
This package provides the runtime container that resolves pre-built `ComponentDefinition[]` arrays generated by [`@goodie-ts/transformer`](https://www.npmjs.com/package/@goodie-ts/transformer). No reflection, no scanning — just a fast topological sort and lazy instantiation.
...
- **`ComponentDefinition<T>`** — Component descriptor with token, scope, factory, dependencies, and metadata
...
- `ApplicationContext.create(defs, options?)` — async factory, evaluates conditional rules (`@ConditionalOnProperty`, `@ConditionalOnEnv`, `@ConditionalOnMissingComponent`) at runtime, then topo-sorts and validates the dependency graph
- `ctx.get(token)` — synchronous lookup (throws for unresolved async components)
- `ctx.getAsync(token)` — always safe for async factories
- `ctx.getAll(token)` — returns all components registered under a token
