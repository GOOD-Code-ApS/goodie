 ...
 *
 * Request-scoped components are resolved from the current request context
 * managed by `RequestScopeManager`. When a singleton depends on a
 * request-scoped component, the transformer generates a proxy that resolves
 * to the current request's instance on each property access.
 *
 * This decorator is a compile-time no-op -- the transformer scans it
 * via AST and sets `scope: 'request'` on the component definition.
 */
