...
export function generateCode(
  ...
  // Config bean is needed when @Value is used OR configDir/inlinedConfig is set
  ...
  const hash = options.hash ?? computeIRHash(beans, options, contributions);
  lines.push(
    `// AppContext.generated.ts — DO NOT EDIT (generated by @goodie-ts/transformer${versionTag} — hash:${hash})`,
  );
  const coreImports = ['ApplicationContext', 'Goodie'];
  // Only import loadConfigFiles when using legacy runtime config loading (no inlinedConfig)
  if (options.configDir && !options.inlinedConfig) {
    coreImports.push('loadConfigFiles');
  }
  lines.push(`import { ${coreImports.join(', ')} } from '@goodie-ts/core'`);
  lines.push("import type { ComponentDefinition } from '@goodie-ts/core'");
  ...
  // If config bean or custom injection tokens are needed
  ...
  // Config token (if @Value is used or configDir is set)
  ...
  if (needsConfigBean) {
    lines.push(
      'export function buildDefinitions(config?: Record<string, unknown>): ComponentDefinition[] {',
    );
    lines.push('  return [');
