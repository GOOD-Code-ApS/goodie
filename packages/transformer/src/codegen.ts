...
export function generateCode(
  ...
  // Config bean is needed when @Value is used OR configDir is set
  const needsConfigBean = hasValueFields || !!options.configDir;
  ...
  if (needsConfigBean) {
    ...
    if (options.configDir) {
      ...
      lines.push(
        `      factory: () => ({ ...loadConfigFiles(${configDirLiteral}, process.env.NODE_ENV), ...process.env, ...config } as Record<string, unknown>),`,
      );
    } else {
      lines.push(
