...
- eb93812: `@ConfigurationProperties` now reads from introspection metadata instead of manual property declarations, leveraging the compile-time type information from `@Introspected`.
- eb93812: Add `@Primary` bean qualifier decorator for disambiguation when multiple components implement the same token.
